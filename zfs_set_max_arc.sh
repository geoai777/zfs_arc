...
#
    ...
    if [[ $1 == "info" ]]; then
        ...
        printf "%-17s geoai777@gmail.com 2024\n\n" " "
    else
        return
    fi
}
...
bv="${BASH_VERSION:-0}"
if [[ `echo $bv | awk -F. '{print $1}'` -ge 4 ]]; then
    msg ack "it is"
else
    msg error "Bash version should be above 4"
fi
...
if [[ -d $pve_dir ]]; then
    ...
    msg error "This is not the prox mox we are looking for..."
    exit 1
fi
...
while true; do
    ...
    msg check "  - Ctrl+C - exit"
    read new_size_human
    if [[ $new_size_human =~ $re ]]; then
        break
    elif [[ $new_size_human -eq "a" ]]; then
        new_size_human=$cache_recommend
        break
    fi
done
